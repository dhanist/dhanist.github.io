...
routing table atau routing information base (RIB). Ya, jadi masing-masing router
...
192.168.1.100, maka paket ini akan diteruskan ke 192.168.1.100. Tapi di mana
router 192.168.1.100 ini? sekali lagi router A cek routing table dan didapat
bahwa 192.168.1.100 masuk network 192.168.1.0/24 dan 192.168.1.0/24 lewat
interface eth0. Dan itulah yang terjadi, paket dengan tujuan 172.16.1.1
diteruskan ke 192.168.1.100 lewat interface eth0.
...
yang disebut administrative distance, semakin kecil nilai distance, dia yang
akan dipilih.

Pengecualian buat Equal Cost Multipath (ECMP) dimana beberapa jalur bisa aktif
...
Nah masing masing AS ini punya nomor yang disebut ASN atau Autonomous System Number.
...
routernya 218.100.36.59 bahwa ada sebanyak 2000 network yang kalau mau menuju
...
multihoming, kita bisa misalnya atur bagaimana caranya salah satu jalur kelihatan
...
Terakhir masalah BGP, semua yang dibahas di atas itu bgp publik, bisa juga
disebut external bgp. BGP juga ada private bgp, yang bisa dipakai siapa saja
...
Ada tiga mode konfigurasi BGP yang bisa dipakai, full mesh, route reflect, sama
route server.
...
Kelemahannya, route reflector jadi spof atau single point of failure. Route
...
Route reflector ini ekstensi iBGP atau internal BGP, semoga masih ingat
tentang internal bgp.
...
biasanya dipakai buat troubleshooting masalah inbound route, coba cek http://www.routeservers.org

Oke, apa yang diperlukan buat setting BGP? kita harus punya:
...
yang seperti ini disebut route redistribution.
...
Bagaimana dengan route-map import? route-map import kita pakai kalau kita perlu
...
Dynamic protocol berikutnya, OSPF. OSPF masuk dalam kategori Interior Gateway
Protocol atau IGP, kategori lainnya EGP atau Exterior Gateway Protocol, BGP di
...
advertising ini di ospf namanya Link State Advertisement atau LSA. Jadi
